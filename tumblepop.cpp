...
void Suck(int& BotState, int bot_type, int bot_index, int& nextinbag, int bag_bot_type[], int bagbot_x[], int bagbot_y[], int bag_bot_index[], int vaccumpositionX, int vaccumpositionY, int direction, int updownvaccum, int& suckingtime, float& bot_x, float& bot_y, int suck_distance, float player_x, float player_y){
...
int ThrownCollision(float& bot_x, float& bot_y, float& throw_velocity_x, float& throw_velocity_y, char** lvl, const int cell_size, int bot_height, int bot_width, const int height, const int width, const float gravity, int updownvaccum, int& throwupdowncounter){

	// Handle upward/downward vacuum platform motion
	...
	if(throwupdowncounter > 0) {
		...
		if(updownvaccum == 1) {
			// Moving upward on top platform
			bot_y -= 5;
		}
		else if(updownvaccum == -1) {
			// Moving downward on bottom platform
			bot_y += 5;
		}
		
		if(throwupdowncounter > 100) {
			throwupdowncounter = 0;
		}
		return 0;  // Don't apply normal physics while on platform
	}
	...
	if(lvl[bot_cell_bottom][bot_cell_left] == '#' || 
	   ...
	   lvl[bot_cell_bottom][bot_cell_right] == '#'||(bot_cell_bottom>850)) {
		throw_velocity_y = 0; // Rebound with energy loss
	...
	if(bot_y <= 0|| (lvl[bot_cell_top][bot_cell_left] == '#' || 
	   ...
	   lvl[bot_cell_top][bot_cell_right] == '#')) {
		throw_velocity_y *= -1; // Rebound with energy loss
	...
	if(bot_x<= 0 && lvl[bot_cell_mid_y][bot_cell_left] == '#') {
		throw_velocity_x *= -0.6; // Rebound with energy loss
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	while (window.isOpen())
	{
		...
		movement_player(ev, Vaccum, vaccumOn, player_x, player_y, player_offset_x, player_offset_y, player_velocityY, player_speed, jumpStrength, isJumping, onGround, go_down, counter_go_down, player_direction,updownvaccum, vaccumframe, startpoint_vaccum, v_startpoint_vaccum, PlayerState);
		...
		for(int i = 0; i<4; i++){
			if(SkeletonState[i]==5) {
				...
				int collision_result = ThrownCollision(
					...
					lvl, cell_size, SkeletonHeight, SkeletonWidth, height, width, gravity, updownvaccum, throwupdowncounter
				);
				...
				if(vaccumOn && SkeletonState[i] != 4&&SkeletonState[i]!=0)
					Suck(SkeletonState[i], 1, i, nextinbag, bag_bot_type, bagbot_x, bagbot_y, bag_bot_index, vaccumpositionX, vaccumpositionY, player_direction, updownvaccum, suckingtime, skeleton_x[i], skeleton_y[i], suck_distance, player_x, player_y);
			}
		}
		for(int i = 0; i<8; i++){
			if(GhostState[i]==5) {
				...
				int collision_result = ThrownCollision(
					...
					lvl, cell_size, GhostHeight, GhostWidth, height, width, gravity, updownvaccum, throwupdowncounter
				);
				if(collision_result == 1) {
				...
				if(vaccumOn && GhostState[i] != 4&&GhostState[i]!=0)
					Suck(GhostState[i], 0, i, nextinbag, bag_bot_type, bagbot_x, bagbot_y, bag_bot_index, vaccumpositionX, vaccumpositionY, player_direction, updownvaccum, suckingtime, ghost_x[i], ghost_y[i], suck_distance, player_x, player_y);
			}
		}
		//Ghost
